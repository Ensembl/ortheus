...
void calculateConstraints_GreaterThan(int64_t *leafPositionsX, int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
                                     struct Constraints ***allConstraints, int64_t *constraints) {
    //Like calculateConstraints, except for 'x-is-greater-than-y' constraints.
    int64_t seqX;
    int64_t seqY;
    int64_t x, x2;
    int64_t dummy; //these values are dummies
...
void calculateConstraints_LessThan(int64_t *leafPositionsX, int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
                                   struct Constraints ***allConstraints, int64_t *constraints) {
...
void makeNonRedundantCollection(struct List *collection, int64_t (*legalCollectionFn)(int64_t *i, int64_t *j, int64_t k), int64_t length) {
    ...
    for(i=0; i<collection->length; i++) {
        ...
        for(j=i+1; j<collection->length; j++) {
            l = collection->list[j];
            if ((!legalCollectionFn(k, l, length)) && memcmp(k, l, length*sizeof(int64_t))) {
                goto outer;
            }
        }
...
void filterCollection(struct List *collection, struct List *collection2, struct List *collection3, int64_t (*legalCollectionFn)(int64_t *i, int64_t *j, int64_t k), int64_t length) {
    //Removes redundancy from a set. Note legalFunction is not necessarily ymmetric.
    int64_t i;
    int64_t j;
    void *k;
...
void mergeNonRedundantCollections(struct List *collection1, struct List *collection2, int64_t (*legalCollectionFn)(int64_t *i, int64_t *j, int64_t k), int64_t length) {
...
int64_t legalCollection_GreaterThan(int64_t *a, int64_t *b, int64_t length) {
    int64_t i;
...
int64_t legalCollection_LessThan(int64_t *a, int64_t *b, int64_t length) {
    int64_t i;
...
int64_t lessThan(int64_t i, int64_t j) {
    return i < j;
}

int64_t greaterThan(int64_t i, int64_t j) {
    return i > j;
}

int64_t edgeFrom(struct Edge *edge) {
    return edge->from;
}

int64_t edgeTo(struct Edge *edge) {
    return edge->to;
}

struct List *cloneCollection(struct List *oldList, int64_t length, struct Chunks *chunks) {
    int64_t i;
    struct List *newList;

    newList = constructEmptyList(oldList->length, NULL);
    for(i=0; i<oldList->length; i++) {
        newList->list[i] = memcpy(mallocChunk(chunks), oldList->list[i], sizeof(int64_t)*length);
    }
    return newList;
}

void getActiveLeaves(int64_t *activeLeaves, int64_t leftMostSeqNo, struct TreeNode *treeNode) {
...
void calculateMergedSequenceCoordinates(int64_t **vertexSequenceCoordinates, struct SequenceGraph *sequenceGraph,
                                        int64_t leftMostSeqNo, int64_t leafSeqNo, void ***mergedVertices, void ***mergedEdges,
                                        int64_t vertexStart, int64_t vertexEnd, int64_t vertexIncrement, int64_t endIncrement,
                                        struct List **connections, int64_t (*legalCollectionFn)(int64_t *, int64_t *, int64_t),
                                        int64_t (*edgeTo)(struct Edge *), int64_t (*lessThan)(int64_t i, int64_t j), struct Chunks *chunks) {
    ...
    int64_t *tempInt;
    int64_t *activeLeaves;
    struct List *sequenceCoordinatesCollection;
    int64_t *sequenceCoordinatesTo;
    int64_t *sequenceCoordinatesFrom;
    int64_t *sequenceCoordinates;
    ...
    activeLeaves = st_malloc(sizeof(int64_t)*leafSeqNo);
        ...
        if (edges->length == 0) {
            ...
            for(i=0; i<edges->length; i++) {
                ...
                if (!edge->silent) {
                    ...
                    memset(activeLeaves, FALSE, sizeof(int64_t)*leafSeqNo);
...
void calculateMergedSequenceCoordinates_LeftToRight(int64_t **vertexSequenceCoordinates, struct SequenceGraph *sequenceGraph, int64_t leftMostSeqNo, int64_t leafSeqNo,
...
void calculateMergedSequenceCoordinates_RightToLeft(int64_t **vertexSequenceCoordinates, struct SequenceGraph *sequenceGraph, int64_t leftMostSeqNo, int64_t leafSeqNo,
                                                   void ***mergedVertices, void ***mergedEdges, struct Chunks *chunks) {
    //As with calculateMergedSequenceCoordinates, but going from left to right
    //instead of right to left
    calculateMergedSequenceCoordinates(vertexSequenceCoordinates, sequenceGraph, leftMostSeqNo, leafSeqNo, mergedVertices, mergedEdges,
...
void **convertSequenceCoordinatesToConstraints(void **mergedSequenceCoordinates, int64_t mergedSequenceCoordinateLength,
                                             int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
                                             struct Constraints ***allConstraints,
                                             void (*constraintsFn)(int64_t *, int64_t , int64_t , int64_t , int64_t , struct Constraints ***, int64_t *),
                                             int64_t (*legalCollectionFn)(int64_t *i, int64_t *j, int64_t length), struct Chunks *chunks) {
...
void **convertSequenceCoordinatesToConstraints_LeftToRight(void **mergedSequenceCoordinates, int64_t mergedSequenceCoordinateLength,
                                                         int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
...
void **convertSequenceCoordinatesToConstraints_RightToLeft(void **mergedSequenceCoordinates, int64_t mergedSequenceCoordinateLength,
                                                         int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
                                                         struct Constraints ***allConstraints, struct Chunks *chunks) {
    return convertSequenceCoordinatesToConstraints(mergedSequenceCoordinates, mergedSequenceCoordinateLength,
                                                   leftMostLeafNoX, leftMostLeafNoY, leafSeqNoX, leafSeqNoY, allConstraints,
...
void calculateMergedConstraints_RightToLeft(int64_t **vertexXSequenceCoordinates,
                                            int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
    ...
    chunks = constructChunks(MEDIUM_CHUNK_SIZE, sizeof(int64_t)*leafSeqNoX);
...
void calculateMergedConstraints_LeftToRight(int64_t **vertexXSequenceCoordinates,
                                            int64_t leftMostLeafNoX, int64_t leftMostLeafNoY, int64_t leafSeqNoX, int64_t leafSeqNoY,
                                            struct Constraints ***allConstraints, struct SequenceGraph *sequenceGraphX,
                                            void ***mergedVertices, void ***mergedEdges, struct Chunks *constraintChunks) {
    void **mergedSequenceCoordinates_Vertices;
...
int64_t *calculateSilentVertices(struct SequenceGraph *sequenceGraph) {
    //labels vertices silent if has (only) silent incoming edges
    int64_t vertex;
    int64_t *silentVertices;
    int64_t i;
    struct Edge *edge;

    silentVertices = st_calloc(sequenceGraph->vertexNo, sizeof(int64_t));
...
int64_t edgeComparator(struct Edge *edge1, struct Edge *edge2) {
    int64_t i;
    int64_t j;
    float k;
    float l;
...
int64_t edgeComparatorStub(struct AlignmentDataStructures *aDS, struct Edge *edge1, struct Edge *edge2) {
    assert(aDS != NULL);
...
void sortEdges(void **list, int64_t length) {
...
struct SequenceGraph *constructSequenceGraph(struct List *edges, int64_t vertexNo) {
    ...
    counts = (int64_t *)st_malloc(sizeof(int64_t)*vertexNo);
    //tos
...
void destructSequenceGraph(struct SequenceGraph *sequenceGraph, int64_t freeIncomingEdges) {
    int64_t i;
...
int64_t *randomChoices(float *probs, int64_t sizeA, int64_t pathWeight) {
    ...
    static int64_t *choices;
    static int64_t choicesSize;
    float randomChoice;
    float totalProb;
    float cumulativeProbs[sizeA];
    int64_t i, j;

    choices = arrayResize(choices, &choicesSize, sizeA, sizeof(int64_t));
    totalProb = probs[0];
...
int64_t getIDFromGraphMemberEdge(void *graphMember) {
    return ((struct Edge *)graphMember)->iD;
}

int64_t getIDfromGraphMemberVertex(void *graphMember) {
    return *((int64_t *)graphMember);
}

void computeUnionScratch(struct AlignmentDataStructures *aDS, struct List *list1, struct List *list2,
                        struct List *scratchList,
                        int64_t (*compare)(struct AlignmentDataStructures *, void *, void *)) {
    int64_t i = 0;
    int64_t j = 0;
    int64_t k = 0;
    int64_t l = 0;
    ...
    int64_t length1 = list1->length;
    int64_t length2 = list2->length;
...
void filterScratch(struct AlignmentDataStructures *aDS, struct List *list,
                  struct List *scratchList,
                  int64_t (*filterFn)(struct AlignmentDataStructures *, void *)) {
    int64_t i = 0;
    int64_t j = 0;
...
int64_t graphMember_EdgeComparator(struct AlignmentDataStructures *aDS, struct GraphMemberHolder *graphMemberHolder1,
...
int64_t graphMember_VertexComparator(struct AlignmentDataStructures *aDS, struct GraphMemberHolder *graphMemberHolder1,
                                 struct GraphMemberHolder *graphMemberHolder2) {
    int64_t i;
...
int64_t edge_graphMember_EdgeComparator(struct AlignmentDataStructures *aDS, struct Edge *edge,
                                    struct GraphMemberHolder *graphMemberHolder) {
    return edgeComparatorStub(aDS, edge, graphMemberHolder->graphMember);
}

int64_t edgeVertexFilter(struct AlignmentDataStructures *aDS, struct Edge *edge) {
    int64_t *i;

    i = bsearch(&(edge->from), aDS->newVertices, aDS->noOfNewVertices, sizeof(int64_t),
                (int (*)(const void *, const void *))intComparator_Int);
    return i != NULL;
}

int64_t greaterThanConditionFilter(struct AlignmentDataStructures *aDS,
                               struct GraphMemberHolder *graphMemberHolder) {
    int64_t i;
    int64_t seq;
    int64_t seqCoordinate;
...
int64_t lessThanConditionFilter(struct AlignmentDataStructures *aDS,
                            struct GraphMemberHolder *graphMemberHolder) {
    int64_t i;
    int64_t j;
    int64_t k;
...
void deleteEndsScratch(struct AlignmentDataStructures *aDS, struct List *previousVertices, struct List *previousEdges,
    ...
    if(sequenceCoordinatesCollection->length == previousSequenceCoordinatesCollection->length) {
        for (i=0; i<sequenceCoordinatesCollection->length; i++) {
            if(memcmp(sequenceCoordinatesCollection->list[i], previousSequenceCoordinatesCollection->list[i], aDS->leafSeqNoX*sizeof(int64_t))) {
                goto outer;
            }
        }
    ...
    divideScratch(aDS, previousEdges, newEdges, newIllegalEdges, (int64_t (*)(struct AlignmentDataStructures *, void *))lessThanConditionFilter);
    ...
    filterScratch(aDS, previousVertices, newVertices, (int64_t (*)(struct AlignmentDataStructures *, void *))lessThanConditionFilter);
    ...
    computeUnionScratch(aDS, newIllegalEdges, previousIllegalEdges, &scratchList1,
                       (int64_t (*)(struct AlignmentDataStructures *, void *, void *))edgeComparatorStub);
    //aDS.newVertices = [ i.graphMember for i in newVertices ]
    aDS->newVertices = arrayResize(aDS->newVertices, &aDS->newVertices_Size, newVertices->length, sizeof(int64_t));
    for (i = 0; i < newVertices->length; ++i) {
        aDS->newVertices[i] = *((int64_t *)((struct GraphMemberHolder *)newVertices->list[i])->graphMember);
    }
    aDS->noOfNewVertices = newVertices->length;
    filterScratch(aDS, &scratchList1, newIllegalEdges, (int64_t (*)(struct AlignmentDataStructures *, void *))edgeVertexFilter);
}

struct Edge *getMinEdge(void** edges, int64_t *size) { //not the best way of doing this, but still
    int64_t i;
    int64_t j;
    int64_t k;
    struct Edge *edge1;
...
int64_t extendEnds_isLegalEnds(struct AlignmentDataStructures *aDS, int64_t *sequenceCoordinates, int64_t *sequenceConstraints) {
    int64_t seq;
...
void extendEnds(struct AlignmentDataStructures *aDS, struct List *vertices, struct List *edges, struct List *illegalEdges,
    ...
    static int64_t newVertices_ListSize;
    int64_t newVertices_List_Index = 0;

    static void **newEdges_List;
    static int64_t newEdges_ListSize;
    int64_t newEdges_List_Index = 0;

    static void **newIllegalEdges_List;
    static int64_t newIllegalEdges_ListSize;
    int64_t newIllegalEdges_List_Index = 0;

    static void **illegalEdges_List;
    static int64_t illegalEdges_ListSize;
    int64_t illegalEdges_List_Index = 0;
    ...
    (*newVertices) = computeUnion(aDS, &temp, vertices, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))graphMember_VertexComparator);

    temp.list = newEdges_List; temp.length = newEdges_List_Index;
    (*newEdges)  = computeUnion(aDS, &temp, edges, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))graphMember_EdgeComparator);
...
void rVZ(struct AlignmentDataStructures *aDS, int64_t c, int64_t *x, int64_t *y, int64_t *z, int64_t *state) { //reverse vertex number function
    int64_t vertexNo;
...
int64_t rVZ_State(struct AlignmentDataStructures *aDS, int64_t c) {
    assert(aDS != NULL);
    return c % stateNo();
}
...
void assignFromTo_Sum(struct AlignmentDataStructures *aDS, int64_t fromState, int64_t toState, float edgeScore) {
    LOG_PLUS_EQUALS(&aDS->toCell[toState], aDS->fromCell[fromState] + edgeScore);
}

int64_t getChanges(int64_t **vertexSequenceCoordinates, struct Edge *edge, int64_t *changes, int64_t leafSeqNo, int64_t leftMostSeqNo) {
    //function to calculate leaf sequence changes along edge
    int64_t i;
    int64_t *sequenceCoordinatesFrom;
    int64_t *sequenceCoordinatesTo;
    int64_t seq;
    static int64_t *activeLeaves;
    static int64_t activeLeavesSize;

    sequenceCoordinatesFrom = vertexSequenceCoordinates[edge->from];
    sequenceCoordinatesTo = vertexSequenceCoordinates[edge->to];

    activeLeaves = arrayResize(activeLeaves, &activeLeavesSize, leafSeqNo, sizeof(int64_t));
    memset(activeLeaves, FALSE, sizeof(int64_t)*leafSeqNo);
    getActiveLeaves(activeLeaves, leftMostSeqNo, edge->treeNode);
    //for seq in xrange(0, aDS.leafSeqNoX) {
    i = 0;
...
int64_t filterDuplicatesFunction(struct List *graphMembers, void **list) {
    int64_t i;
    int64_t j;
    void *previous;
    void *current;
...
void debugSets(struct AlignmentDataStructures *aDS, struct List *newEdgesPointer, struct List *newVerticesPointer, struct List *illegalEdgesPointer) {
    if(DEBUG) { //debug code
        ...
        for(xx=0; xx<newEdgesPointer->length; xx++) {
            ...
            if (yy->to == xxx) {
                ...
                if(edgeComparator(pEdge, yy) == 0) {
                    if(memcmp(prev, ((struct GraphMemberHolder *)newEdgesPointer->list[xx])->sequenceConstraints, aDS->leafSeqNoX*sizeof(int64_t)) == 0) {
        ...
        for(xx=0; xx<newVerticesPointer->length; xx++) {
            int64_t yy = *((int64_t *)((struct GraphMemberHolder *)newVerticesPointer->list[xx])->graphMember);
            assert(xxx <= yy);
            if(xxx == yy) {
                if(memcmp(prev, ((struct GraphMemberHolder *)newVerticesPointer->list[xx])->sequenceConstraints, aDS->leafSeqNoX*sizeof(int64_t)) == 0) {
...
void computeMatrix(struct AlignmentDataStructures *aDS) {
    ...
    rightMostVertices = st_calloc(aDS->sequenceGraphX->vertexNo, sizeof(int64_t));
    ...
    for (toX=0; toX < aDS->sequenceGraphX->vertexNo; toX++) {
        if (aDS->sequenceGraphXSilentVertices_To[toX]) {
            ...
            for (i=0; i< aDS->sequenceGraphX->edgesArrangedByToVertex[toX]->length; i++) {
                ...
                computeUnionScratch(aDS, &filteredVertices, &newVertices, &scratchList1, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))graphMember_VertexComparator);
                swapListFields(&newVertices, &scratchList1);

                computeUnionScratch(aDS, &filteredEdges, &newEdges, &scratchList1, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))graphMember_EdgeComparator);
                swapListFields(&newEdges, &scratchList1);

                computeUnionScratch(aDS, &filteredIllegalEdges, &illegalEdges, &scratchList1, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))edgeComparatorStub);
                swapListFields(&illegalEdges, &scratchList1);
            }
            diffScratch(aDS, &illegalEdges, &newEdges, &scratchList1, (int64_t (*)(struct AlignmentDataStructures *, void *, void *))edge_graphMember_EdgeComparator);
            swapListFields(&illegalEdges, &scratchList1);
            ...
            for (i=0; i< aDS->sequenceGraphX->edgesArrangedByToVertex[toX]->length; i++) {
                ...
                filterScratch(aDS, previousVertices[fromX], &scratchList1, (int64_t (*)(struct AlignmentDataStructures *, void *))greaterThanConditionFilter);
                filterScratch(aDS, previousEdges[fromX], &scratchList2, (int64_t (*)(struct AlignmentDataStructures *, void *))greaterThanConditionFilter);
...
void assignSampling(struct AlignmentDataStructures *aDS,
        int64_t fromState, int64_t toState, float edgeScore, float correctedEdgeScore) {
    float *fromVertices;
...
struct TreeNode *getTreeNode_insertX(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID) {
    return copyConstructTreeNode(TREE_NODE_INSERT, transitionID,
                                 aDS->traversalID, traceBackEdge->edgeX->treeNode, NULL, NULL);
}

struct TreeNode *getTreeNode_deleteX(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID, float *wV) {
...
struct TreeNode *getTreeNode_silentX(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID) {
    return copyConstructTreeNode(TREE_NODE_PREVIOUSLY_SILENT, transitionID,
                                 aDS->traversalID, traceBackEdge->edgeX->treeNode, NULL, NULL);
}

struct TreeNode *getTreeNode_insertY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID) {
    return copyConstructTreeNode(TREE_NODE_INSERT, transitionID,
                                 aDS->traversalID, traceBackEdge->edgeY->treeNode, NULL, NULL);
}

struct TreeNode *getTreeNode_deleteY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID, float *wV) {
...
struct TreeNode *getTreeNode_silentY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID) {
    return copyConstructTreeNode(TREE_NODE_PREVIOUSLY_SILENT, transitionID,
                              aDS->traversalID, traceBackEdge->edgeY->treeNode, NULL, NULL);
}

struct TreeNode *getTreeNode_matchXY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID, float *wV) {
...
struct TreeNode *getTreeNode_deleteXY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID, float *wV) {
...
struct TreeNode *getTreeNode_silentXY(struct AlignmentDataStructures *aDS, struct TraceBackEdge* traceBackEdge, int64_t transitionID) {
    assert(traceBackEdge != NULL);
    return copyConstructTreeNode(TREE_NODE_SILENT, transitionID, aDS->traversalID, NULL, NULL, NULL);
}

void updateIndices(struct TreeNode *treeNode, int64_t *currentIndices, int64_t leftMostLeafNo) {
...
int64_t **calculateVertexSequenceCoordinates(struct SequenceGraph *sequenceGraph, int64_t leftMostLeafNo, int64_t leafSeqNo, struct Chunks *chunks) {
    int64_t vertex;
    struct Edge *previousEdge;
    int64_t **sequenceCoordinates;
    int64_t *currentIndices;
    ...
    sequenceCoordinates = st_malloc(sizeof(int64_t *)*sequenceGraph->vertexNo); //{ 0:[-1]*leafSeqNo }
    sequenceCoordinates[0] = mallocChunk(chunks); //callocLocal(leafSeqNo, sizeof(INT_32));
    memset(sequenceCoordinates[0], 0, sizeof(int64_t) * leafSeqNo);
    for(vertex=1; vertex<sequenceGraph->vertexNo; vertex++) { // in xrange(1, sequenceGraph.vertexNo):
        ...
        currentIndices = memcpy(mallocChunk(chunks), sequenceCoordinates[previousEdge->from], sizeof(int64_t)*leafSeqNo);
...
struct SequenceGraph *compactSilentVertices(struct SequenceGraph *sequenceGraph) {
    ...
    int64_t vertexShift = 0;
    int64_t *verticeShifts;
    struct Edge *edge;
    int64_t i;
    struct SequenceGraph *finalSequenceGraph;

    verticeShifts = st_calloc(sequenceGraph->vertexNo, sizeof(int64_t));
...
struct SequenceGraph* convertTraceBackEdgesToSequenceGraph(struct AlignmentDataStructures *aDS, void **newEdges, int64_t newEdgeNumber) {
    int64_t i;
    ...
    intChunks = constructChunks(newEdgeNumber + 1, sizeof(int64_t));
    ...
    for(i=0; i<newEdgeNumber; i++) {
        ...
        traceBackEdge->from = *((int64_t *)hashtable_search(vertexMap, &traceBackEdge->from));
        if(traceBackEdge->silent) {
            //treeNode = ((struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge *, int64_t))traceBackEdge->getTreeNode)(aDS, traceBackEdge, transitionID);
            ...
            treeNode = ((struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge *, int64_t, float *))traceBackEdge->getTreeNode)(aDS, traceBackEdge, transitionID, wV);
            edge = copyConstructEdge(traceBackEdge->from, traceBackEdge->to, traceBackEdge->edgeScore,
...
struct SequenceGraph *traceBackMatrix(struct AlignmentDataStructures *aDS) {
    ...
    static int64_t newEdgesSize;
    int64_t newEdgesIndex = 0;
    ...
    while (!heap_empty(vertexHeap)) {
        ...
        if (aDS->sequenceGraphXSilentVertices_To[toX]) {
            ...
            aDS->getTreeNode = (struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge*, int64_t))getTreeNode_deleteY;
            //for edgeX in edgesX {
            for (i = 0; i < edgesX->length; ++i) {
                aDS->edgeX = edgesX->list[i];
                fromX = aDS->edgeX->from;
            ...
            aDS->getTreeNode = (struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge*, int64_t))getTreeNode_deleteX;
            //for edgeY in edgesY {
            for (i = 0; i < edgesY->length; ++i) {
                aDS->edgeY = edgesY->list[i];
                fromY = aDS->edgeY->from;
            ...
            aDS->getTreeNode = (struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge*, int64_t))getTreeNode_matchXY;
            //for edgeX in edgesX {
            for (i = 0; i < edgesX->length; ++i) {
                aDS->edgeX = edgesX->list[i];
                fromX = aDS->edgeX->from;
            ...
            aDS->getTreeNode = (struct TreeNode *(*)(struct AlignmentDataStructures *, struct TraceBackEdge*, int64_t))getTreeNode_deleteXY;
...
int64_t wrapInGraphHolders_Length;
int wrapInGraphHolders_intComp(int64_t **iAA, int64_t **iAA2) {
    int64_t i;
    int64_t *iA = *iAA;
    int64_t *iA2 = *iAA2;
...
void wrapInGraphHolders(struct AlignmentDataStructures *aDS, struct List *list, void *key, int64_t(*comp)(struct AlignmentDataStructures *, struct GraphMemberHolder *, struct GraphMemberHolder *)) {
    int64_t i;
...
void prepareGraphForAlignment(struct SequenceGraph *sequenceGraphX, struct CombinedTransitionModel *model, int64_t flipped) {
    int64_t i;
    struct Edge *edge;
...
struct SequenceGraph *computeEdgeGraph(struct SequenceGraph *sequenceGraphX, struct SequenceGraph *sequenceGraphY,
    ...
    struct Chunks *intChunks;
    int64_t **vertexYSequenceCoordinates;
    ...
    mergedChunksX = constructChunks(MEDIUM_CHUNK_SIZE, sizeof(int64_t)*aDS->leafSeqNoX);
    mergedChunksY = constructChunks(MEDIUM_CHUNK_SIZE, sizeof(int64_t)*aDS->leafSeqNoY);
    ...
    intChunks = constructChunks(sequenceGraphY->vertexNo, sizeof(int64_t));
    ...
    aDS->newVertices = st_malloc(sizeof(int64_t) * aDS->newVertices_Size);
    aDS->noOfNewVertices = 0;
    aDS->changes = st_malloc(sizeof(int64_t) * leafSeqNoX * 2);
    aDS->noOfChanges = 0;
...
int64_t tranCoord(int64_t from, int64_t to, int64_t node, int64_t stateNo, int64_t nodeNo) {
...
float *upPass(struct TreeNode *treeNode, float *working, struct SubModel **subModels, float *totalP,  int64_t alphabetSize);

void branchUp(struct TreeNode *treeNode, float *working, float *currentResult, struct SubModel **subModels, float *totalP,  int64_t alphabetSize) {
    float *i;
    int64_t j;
...
void downPass(struct TreeNode *treeNode, float *ancestorProb, float *working, float *results, struct SubModel **subModels, float *totalP, float totalAncP, float globalTotalP,  int64_t alphabetSize);

void branchDown(struct TreeNode *treeNode, float *ancestorProbs, float *working, float *results, struct SubModel **subModels, float *totalP, float totalAncP, float globalTotalP, int64_t alphabetSize) {
...
void downPass(struct TreeNode *treeNode, float *ancestorProbs, float *working, float *results,
              struct SubModel **subModels, float *totalP, float totalAncP, float globalTotalP, int64_t alphabetSize) {
    float *i;
    float *j;
    float *k;
    float l;
...
struct SequenceGraph *align_BottomUpScript(struct BinaryTree *binaryTree, struct SequenceGraph **sequenceGraphs, struct Constraints ***constraints,
    ...
    if (binaryTree->internal) {
        ...
        st_logInfo("sequence graph successfull\n");
